...
async function checkAccessibilityWithAxe(page) {
  try {
    ...
    // 실행 컨텍스트 오류 방지를 위한 추가 대기
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    // axe-core 라이브러리 삽입 - CDN 연결 오류 대비하여 try-catch로 감싸기
    try {
      ...
      if (!axeLoaded) {
        console.error(chalk.default.red('axe-core 라이브러리가 로드되지 않았습니다'));
        return {
          passes: [],
          violations: [],
          incomplete: [],
      ...
      const results = await page.evaluate(() => {
        ...
        console.error(chalk.default.yellow('axe-core 실행 중 오류:'), err.message);
        return {
          error: err.message,
          passes: [],
          violations: [],
      ...
      console.error(chalk.default.red('axe-core 스크립트 로드 실패:'), scriptError);
      
    }
  } catch (error) {
    console.error(chalk.default.red('axe-core 접근성 검사 중 오류가 발생했습니다:'), error);
    return {
      passes: [],
      violations: [],
      incomplete: [],
...
//     jsxContent += `
//       <h2>통과한 항목</h2>
//       <ul className="a11y-passes">
//         ${passes.map((pass, index) => `
//         <span key="${index}">
//           <strong>${pass.id || '알 수 없음'}:</strong> ${pass.description || pass.help || '정보 없음'}
//         </span>`).join('')}
//       </ul>
//     `;
//   }
...
function generateSimpleTextReport(results, componentName) {
  ...
  if (violations.length > 0) {
    ...
    violations.forEach((violation, index) => {
      ...
      reportText += `- **설명**: ${violation.description || violation.help || '정보 없음'}\n`;
      reportText += `- **도움말**: [자세히 보기](${violation.helpUrl || '#'})\n\n`;
      ...
      if (violation.nodes && violation.nodes.length > 0) {
        ...
        violation.nodes.forEach((node, nodeIndex) => {
          ...
          reportText += '```\n' + html + '\n```\n\n';
          reportText += `- **해결 방법**: ${node.failureSummary || '정보 없음'}\n\n`;
        });
      }
    });
  }
  ...
  if (passes.length > 0) {
    ...
    passes.forEach((pass, i) => {
      reportText += `- **${pass.id || '알 수 없음'}**: ${pass.description || pass.help || '정보 없음'}\n`;
    });
    reportText += '\n';
  }
  ...
  if (incomplete.length > 0) {
    ...
    incomplete.forEach((item, i) => {
      reportText += `- **${item.id || '알 수 없음'}**: ${item.description || item.help || '정보 없음'}\n`;
    });
    reportText += '\n';
  }
...
// 접근성 보고서 파일 저장
async function saveA11yReport(componentName, jsxContent, cssContent, reportText, axeResults) {
  try {
    ...
    // 컴포넌트명을 파일명에 포함시키기
    // CSS 파일 저장
    const cssFilePath = path.join(a11yFolderPath, `${actualComponentName}-report.css`);
    fs.writeFileSync(cssFilePath, cssContent);
    
    // 백업용 마크다운 파일 저장
    const mdFilePath = path.join(a11yFolderPath, `${actualComponentName}-report.md`);
    fs.writeFileSync(mdFilePath, reportText);
    
    // JSON 데이터 저장 - 컴포넌트명-data.json 파일로 저장
    const jsonFilePath = path.join(a11yFolderPath, `${actualComponentName}-data.json`);
    ...
    const viewComponentSource = path.join(a11yFolderPath, `${actualComponentName}View.jsx`);
...
export default ${actualComponentName}View;`;
    ...
    const storiesComponentPath = path.join(a11yFolderPath, `${actualComponentName}View.stories.jsx`);
...
A11yReport.storyName = '${actualComponentName} 검사 결과';`;
    ...
    console.log(chalk.default.yellow(`A11y 폴더 위치: ${a11yFolderPath} (Components 폴더와 같은 레벨)`));
    console.log(chalk.default.blue(`스토리북에서 볼 수 있는 경로: ${mainComponentName}/A11y/${actualComponentName} 검사 결과`));
    ...
    console.error(chalk.default.red('접근성 보고서 저장 중 오류가 발생했습니다:'), error);
    return null;
  }
}
...
async function getComponentHTML(componentName, storyId = 'default') {
  ...
  try {
    ...
    console.log(chalk.default.blue(`컴포넌트 HTML을 가져오는 중... URL: ${url}`));
    ...
    // 타임아웃 시간 증가 및 네트워크 요청 대기
    await page.goto(url, { 
      waitUntil: 'networkidle2',
      timeout: 60000 // 60초로 타임아웃 증가
    });
    
    // 페이지 로딩 및 렌더링 완료 대기 - 더 긴 대기 시간
    await new Promise(resolve => setTimeout(resolve, 5000)); // 5초로 늘림
    ...
    if (!isRendered.success) {
      console.error(chalk.default.red('컴포넌트가 제대로 렌더링되지 않았습니다.'));
      console.error(chalk.default.red('스토리북 스토리 ID와 경로를 확인하세요.'));
      // 렌더링 실패 시에도 계속 진행
    } else {
      console.log(chalk.default.green(`컴포넌트가 렌더링되었습니다. (셀렉터: ${isRendered.selector})`));
    }
    ...
    if (browser) {
      try {
        await browser.close();
        console.log(chalk.default.blue('브라우저 세션이 정상적으로 종료되었습니다.'));
      } catch (closeError) {
        console.error(chalk.default.yellow('브라우저 종료 중 오류 (무시됨):'), closeError.message);
      }
    }
  }
}
...
async function main() {
  try {
    ...
    if (args.length === 0) {
      ...
      console.log(chalk.default.yellow('예시: npm run a11y Button/Components/Button'));
      console.log(chalk.default.yellow('스토리북 경로와 동일하게 입력하세요 (예: Button/Components/Button)'));
      process.exit(1);
    }
    ...
    console.log(chalk.default.yellow(`${componentName} 컴포넌트의 접근성 검사를 시작합니다...`));
    console.log(chalk.default.yellow(`스토리북 서버가 포트 6007에서 실행 중인지 확인하세요. (npm run storybook)`));
    
    // 컴포넌트 HTML 가져오기 및 axe-core로 검사
    const result = await getComponentHTML(componentName);
    
    if (!result) {
      console.error(chalk.default.red('컴포넌트 HTML을 가져오지 못했습니다. 스토리북 서버가 실행 중인지 확인하세요.'));
      process.exit(1);
    }
    
    // 1. React 컴포넌트 생성 (백업용)
    const jsxContent = generateReportComponent(result.axeResults, componentName);
    
    // 2. CSS 스타일 생성 (백업용)
    const cssContent = generateReportStyle();
    
    // 3. 단순 텍스트 형식의 보고서 생성
    const textReport = generateSimpleTextReport(result.axeResults, componentName);
    
    // 파일 저장
    await saveA11yReport(componentName, jsxContent, cssContent, textReport, result.axeResults);
    ...
    if (violations.length > 0) {
      console.log(chalk.default.yellow(`경고: ${violations.length}개의 접근성 위반 항목이 발견되었습니다.`));
      console.log(chalk.default.yellow('스토리북에서 접근성 보고서를 확인하세요.'));
    } else {
      console.log(chalk.default.green('모든 접근성 검사를 통과했습니다!'));
    }
    
    // 통과한 항목을 한국어로 콘솔에 출력
    // const passes = result.axeResults.passes || [];
    // if (passes.length > 0) {
    //   console.log(chalk.default.green('\n통과한 항목:'));
    //   passes.forEach((pass, i) => {
    //     const koreanDescription = getKoreanDescriptionForPass(pass.id, pass.description || pass.help || '정보 없음');
    //     console.log(chalk.default.green(`  - ${pass.id}: ${koreanDescription}`));
    //   });
    // }
  } catch (error) {
    console.error(chalk.default.red('접근성 검사 중 오류가 발생했습니다:'), error);
    process.exit(1);
  }
}
...
function getKoreanDescriptionForPass(id, originalDescription) {
  const koreanDescriptions = {
    ...
    'aria-hidden-focus': 'aria-hidden 요소는 포커스 가능한 요소를 포함하지 않으며 포커스가 불가능합니다',
    'color-contrast': '전경색과 배경색 간의 대비가 WCAG 2 AA 최소 대비율 기준을 충족합니다',
    'link-in-text-block': '링크가 주변 텍스트와 색상에만 의존하지 않고 구분됩니다',
